...
L<virt-v2v-release-notes-2.12(1)> — Release notes for 2.12 release.
...
=item B<--block-driver> B<none>

=item B<--block-driver> B<virtio-blk>

=item B<--block-driver> B<virtio-scsi>

When choosing a block driver for Windows guests, prefer C<virtio-blk>
or C<virtio-scsi>, or do not pre-install any storage driver.

The default is C<virtio-blk>.

This only affects pre-installation of storage drivers for Windows
guests.  If virtio-win drivers are available, they will still be
copied into the guest, even if C<none> is selected here.

This has no effect for Linux guests.
')
...
(This issue is fixed in libguestfs E<ge> 1.60, the following
documentation is for historical versions.)
